...
Enable **Translucent main window** in Appearance settings to see a softly blurred background behind Freemind. The opacity slider adjusts the main window's content from 65% to 100%, including native editors and terminals. Settings and detached workspace and terminal windows stay opaque. The option is off by default, updates immediately, and respects macOS **Reduce Transparency**.
...
The workspace tree uses native disclosure rows, hiding the disclosure control for empty workspaces. Hook events drive the busy spinner and completion status. Codex's terminal notifications show a yellow attention indicator and play the macOS Glass sound once when a response finishes or Codex needs user input. Routine tool calls, automatic permission checks, and subagent hooks stay quiet. Activity clears the attention indicator. Updated notification settings apply when a Codex process is next launched; reconnecting to an existing process preserves its launch settings. File, Git, Notes, and editor divider positions are saved with the workspace.
